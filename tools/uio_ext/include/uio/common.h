 ...
 */
#ifndef __UIO_EXT_COMMON_H
#define __UIO_EXT_COMMON_H
...
#define UIO_BUG(__fmt, ...)							\
	do {									\
		fprintf(stderr, "%s:%d [uio panic]: %s\n", __FILE__, __LINE__,	\
			strerror(errno));					\
...
#define UIO_BUG_ON(__cond, __fmt, ...)					\
	do {								\
		if (__cond)						\
			UIO_BUG((__fmt) __VA_OPT__(,) __VA_ARGS__);	\
	} while (0)
